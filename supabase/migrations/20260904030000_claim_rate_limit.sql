-- Review P0 #8 (part): rate-limit claims at the database, where it can't be
-- dodged by calling the RPC directly. A user gets 8 claim attempts and 5
-- verification emails per day — plenty for a real trader with a few codes,
-- useless for enumerating the registry or spamming registered contacts
-- through our sender domain.
...
begin
  ...
  if (select count(*) from user_holders
       where user_id = v_uid and granted_at > now() - interval '1 day') >= 8 then
  ...
  if v_status = 'pending' and v_reg.contact_email is not null and v_is_server then
    if (select count(*) from holder_verifications
         where user_id = v_uid and created_at > now() - interval '1 day') >= 5 then
...
-- Belt-and-suspenders on the secret stores (adversary #3): deny at the GRANT
-- layer so a future careless `using (true)` policy exposes nothing.
revoke all on crrah_registry from anon, authenticated;
revoke all on holder_verifications from anon, authenticated;
